  ...
  useEffect(() => {
    const fetchGuides = async () => {
      try {
        ...
        setError(t('travelGuides.loadError'));
        setLoading(false);
      }
    };
    fetchGuides();
  ...
  if (error) return (
    <div className="error-container">
      <div className="error-message">
        <p className="error-title">{t('travelGuides.oops')}</p>
        <p>{error}</p>
      </div>
    </div>
  );
    ...
    <div className="guides-page-container">
      <div className="guides-content-wrapper">
        ...
        <div className="guides-header">
          <h1 className="guides-title">{t('travelGuides.title')}</h1>
          <p className="guides-subtitle">{t('travelGuides.subtitle')}</p>
        </div>
        ...
        <div className="guides-filters">
          <div className="search-input-wrapper">
            ...
            <input
              type="text"
              placeholder={t('travelGuides.searchPlaceholder')}
          ...
          <div className="location-select-wrapper">
            <select className="location-select" value={locationFilter}
              onChange={(e) => setLocationFilter(e.target.value)}>
              <option value="">{t('travelGuides.allLocations')}</option>
              ...
              <div className="h-64 w-full relative overflow-hidden bg-gray-200">
                ...
                <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                  ...
                  <div className="bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-lg shadow-lg flex items-center gap-1.5 font-bold text-sm text-gray-800">
                    ...
                    {guide.rating > 0 ? guide.rating : t('travelGuides.new')}
                  </div>
                </div>
              </div>
              ...
              <div className="p-7">
                <div className="flex justify-between items-center mb-6 py-3 border-b border-gray-100">
                  <div className="text-gray-500 text-sm font-medium">
                    {guide.reviews > 0
                      ? <>{guide.reviews} {t('travelGuides.reviews')}</>
                      : t('travelGuides.noReviews')}
                  </div>
                  <div className="text-right">
                    <span className="text-2xl font-black text-indigo-600">₹{guide.price || 1500}</span>
                    <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide ml-1">{t('travelGuides.perDay')}</span>
                  </div>
                </div>
                ...
                <div className="space-y-5 mb-8">
                  {guide.specialties?.length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">{t('travelGuides.specialties')}</p>
                  ...
                  <div>
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{t('travelGuides.languages')}</p>
                    <p className="text-sm font-medium text-gray-700 flex items-center gap-2">
                      <svg className="w-4 h-4 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129" />
                      </svg>
                      {guide.languages?.length > 0 ? guide.languages.join(", ") : t('travelGuides.defaultLanguages')}
                    </p>
                  </div>
                </div>

                <button className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-3.5 px-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 transform active:scale-95">
                  {t('travelGuides.viewProfile')}
                </button>
              </div>
            </div>
          ))}
        ...
        {filteredGuides.length === 0 && !loading && (
          <div className="text-center py-20 bg-white rounded-3xl shadow-sm border border-gray-100">
            ...
            <p className="text-xl font-semibold text-gray-600">{t('travelGuides.noGuides')}</p>
            <p className="text-gray-400 mt-2">{t('travelGuides.noGuidesHint')}</p>
          </div>
        )}
