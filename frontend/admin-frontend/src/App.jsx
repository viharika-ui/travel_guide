...
import GuideContextProvider, { useGuide } from "./context/GuideContext";
...
function GuideLayout() {
  const { guideData, logout } = useGuide();
    ...
    <div style={{ display: "flex", height: "100vh", background: C.bg, fontFamily: "'Inter', 'Segoe UI', sans-serif", color: C.text, overflow: "hidden" }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: "20px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12, minHeight: 68 }}>
          ...
          {!collapsed && <div style={{ color: C.text, fontWeight: 800, fontSize: 16, whiteSpace: "nowrap" }}>Guide Portal</div>}
        </div>
        ...
        <nav style={{ flex: 1, padding: "12px 8px", overflowY: "auto", overflowX: "hidden" }}>
          {GUIDE_NAV.map(n => (
            <button key={n.id} onClick={() => setActive(n.id)} style={{
              ...
              background: active === n.id ? C.purple + "18" : "transparent",
              color: active === n.id ? C.purple : C.muted,
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <header style={{
          ...
          <div style={{ color: C.text, fontWeight: 700, fontSize: 16 }}>
            {GUIDE_NAV.find(n => n.id === active)?.icon} {GUIDE_NAV.find(n => n.id === active)?.label}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: "7px 14px" }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: C.purple + "30", color: C.purple, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 13 }}>
                {guideData?.name?.[0]?.toUpperCase() || "G"}
              </div>
              <span style={{ color: C.text, fontSize: 13, fontWeight: 600 }}>{guideData?.name || "Guide"}</span>
              <button onClick={() => { logout(); window.location.href = '/guide/login'; }} style={{ background: "none", border: "none", color: C.danger, cursor: "pointer", fontSize: 13, fontWeight: 600, padding: "0 0 0 8px", borderLeft: `1px solid ${C.border}`, marginLeft: 4 }}>
                Logout
              </button>
            </div>
          </div>
...
function AppInner() {
  ...
  return (
    <Routes>
      {/* Root redirects to admin login by default or dashboard if logged in */}
      <Route path="/" element={<Navigate to="/admin/login" />} />

      {/* Admin Routes */}
      <Route path="/admin/login" element={admin ? <Navigate to="/admin/dashboard" /> : <AdminLogin />} />
      <Route path="/admin/dashboard" element={admin ? <Layout /> : <Navigate to="/admin/login" />} />
      <Route path="/admin/*" element={admin ? <Layout /> : <Navigate to="/admin/login" />} />

      {/* Guide Routes */}
      <Route path="/guide/login" element={gToken ? <Navigate to="/guide/dashboard" /> : <GuideLogin />} />
      <Route path="/guide/register" element={gToken ? <Navigate to="/guide/dashboard" /> : <GuideRegister />} />
      <Route path="/guide/dashboard" element={gToken ? <GuideLayout /> : <Navigate to="/guide/login" />} />
      <Route path="/guide/*" element={gToken ? <GuideLayout /> : <Navigate to="/guide/login" />} />
    </Routes>
  );
}
